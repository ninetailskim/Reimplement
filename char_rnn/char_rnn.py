...
class Model(ModelDesc):
    ...
    def build_graph(self, input, nextinput):
        #two layers
        cell = rnn.MultiRNNCell([rnn.LSTMBlockCell(num_units=param.rnn_size) for _ in range(param.num_rnn_layer)])

        def get_v(n):
            ret = tf.get_variable(n + '_unused', [param.batch_size, param.rnn_size],
                                    trainable=False, initializer=tf.constant_initializer())
            #placeholder, output the default
        ...
        print("last_state shape", len(last_state))
        print(last_state)
        last_state = tf.identity(last_state, 'last_state')

        output = tf.reshape(tf.concat(outputs, 1), [-1, param.rnn_size])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corpus', help='corpus file', default='input.txt')
    subparsers = parser.add_subparsers(title='command', dest='command')
    parser_sample = subparsers.add_parser('sample', help='sample a trained model')

    parser_sample.add_argument('-n','--num', type=int, default=300, help='length of text to generate')
    parser_sample.add_argument('-s','--start',default='I am ', help='initial text sequence')
    parser_sample.add_argument('-t','--temperature', type=float, default=1, help='softmax temperature')
